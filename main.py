...
def get_price(soup, interest):
    ...
    if url_type == 'www.forever21.com':
        price = soup.find(class_='price').find_all(class_='value')[1].text.replace(',', '').replace('RUB', '')
        end_price = f'{(float(price) * (1 + interest))} RUB'

    elif url_type == 'aliexpress.ru':
        price = soup.find(class_='product-price-current').text.replace('US $', '')
        end_price = '$' + str(float(price) * (1 + interest))
        ...
        price = soup.find(class_='val').find('span', class_='notranslate').text.replace('US $', '').replace(',', '.')
        ...
        if delivery != 'БЕСПЛАТНО':
            ...
            delivery = 0
        end_price = f'${((float(price)) * (1 + interest) + delivery)}'
